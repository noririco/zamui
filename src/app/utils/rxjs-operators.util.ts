 ...
 * @param name
 */
export const ObsLog = <T>(source: Observable<T>, name: string) =>
  defer(() => {
    ...
    return source.pipe(
      tap({
        ...
        error: (error) =>
          console.log(`%c Error: `, 'background: #222; color: #db0b00', error),
        complete: () =>
          console.log(
            `%c ${name}: complete`,
            'background: #222; color: #bada55'
...
export function testReg(regex: RegExp): OperatorFunction<string, boolean> {
  return (inputObs$) => inputObs$.pipe(map((_str: string) => regex.test(_str)));
}
 ...
 *
 */
export function filterRegex<T, K extends keyof T>(
  key: K,
  regex: RegExp
 ...
 */
export function filterLogical<T, K extends keyof T>(
  key: K,
  logicalExp: string
): OperatorFunction<T[], T[]> {
  return (inputObs$) =>
